 ...
 ***********************************************/
    ...
    static void initFile(MatAxD atoms) {
        ...
        stdio.fprintf([handle__1=handle],"%d\n(0,0)AtomConfig\n", NUM_ATOMS);
        handle__2 = writeRec(atoms, handle__1, 0);
        stdio.fclose([handle__close = handle__2]);
    ...
    static FILE writeRec(MatAxD atoms, FILE handle, int pos) {
        if (pos == NUM_ATOMS) {
            ...
            handle__res = writeAtom(atoms.rowVec(pos), handle);
            cancel writeRec(atoms, handle__res, pos + 1);
        }
    }

    // Wrapper for writing an atom to the File
    static FILE writeAtom(VecATOMS_row atoms, FILE handle) {
        stdio.fprintf([handle__ret = handle], "H\t%f\t%f\t%d\n", atoms.one_d[0], atoms.one_d[1], 0);
        cancel handle__ret;
    }
}
