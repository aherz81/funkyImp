...
float ocl_kernel::getRunTime(int i){
    ...
    return (end-start)*1.e-3;
}

cl_ulong ocl_kernel::getRunTimeNS(int i) {
    cl_event event;
    cl_ulong start,end;
...
std::string ocl_device::getName() {
    char buffer[256];
    size_t writtenBytes;
    clGetDeviceInfo(dID, CL_DEVICE_NAME, 256, buffer, &writtenBytes);
    return std::string(buffer);
}

cl_device_type ocl_device::getDeviceType() {
    cl_device_type devType;
    clGetDeviceInfo(dID, CL_DEVICE_TYPE, sizeof(cl_device_type), &devType, NULL);
    return devType;
}
