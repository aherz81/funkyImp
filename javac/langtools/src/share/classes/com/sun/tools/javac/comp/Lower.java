    ...
    class EnumMapping {
        EnumMapping(DiagnosticPosition pos, TypeSymbol forEnum) {
            ...
            this.mapVar = new VarSymbol(STATIC | SYNTHETIC | FINAL,
                                        varName,
                                        new ArrayType(syms.intType,null, syms.arrayClass),
                                        outerCacheClass);
        ...
        void translate() {
            ...
            JCExpression mapVarInit = make
                .NewArray(make.Type(syms.intType),null, List.of(size), null)
                .setType(new ArrayType(syms.intType,null,syms.arrayClass));
 ...
 *************************************************************************/
        ...
        case TYP:
            if (sym.owner.kind != PCK) {
                ...
                if (tree.getTag() == JCTree.IDENT) {
                    ...
                    ((JCFieldAccess) tree).name = make.Ident(flatname);
                }
            }
            break;
        case MTH: case VAR: //ALEX exclude lookup for domain vars
            if (sym.owner.kind == TYP && sym.owner.type.tag!=DOMAIN) {
    ...
    void makeAccessible(Symbol sym) {
		if((sym.flags_field&Flags.BLOCKING)!=0)
			return;
...
//      System.err.println("access " + vsym + " with " + accessor);//DEBUG
        ...
        if (sym.kind == VAR) {
            ...
            stat = make.Return(expr.setType(sym.type),0);
        } else {
    ...
    JCBlock classDollarSymBody(DiagnosticPosition pos, JCMethodDecl md) {
        ...
        if (target.classLiteralsNoInit()) {
            ...
            JCNewArray newcache = make.
                NewArray(make.Type(outerCacheClass.type),null,
                         List.<JCExpression>of(make.Literal(INT, 0).setType(syms.intType)),
                         null);
            newcache.type = new ArrayType(types.erasure(outerCacheClass.type),null,
                                          syms.arrayClass);
                ...
                make.Conditional(
                    ...
                    make.Assign(
                        ...
                        makeCall(
                            ...
                            List.<JCExpression>nil())).setType(null),
                    make.Ident(clsym)).setType(null);
 ...
 *************************************************************************/
    ...
    private void visitEnumDef(JCClassDecl tree) {
        ...
        Type arrayType = new ArrayType(types.erasure(tree.type),null,syms.arrayClass);
        VarSymbol valuesVar = new VarSymbol(PRIVATE|FINAL|STATIC|SYNTHETIC,
                                            valuesName,
                                            arrayType,
                                            tree.type.tsym);
        JCNewArray newArray = make.NewArray(make.Type(types.erasure(tree.type)), null,
                                          List.<JCExpression>nil(),
                                          values.toList());
        ...
        if (useClone()) {
            ...
            valuesBody = List.<JCStatement>of(make.Return(valuesResult,0));
        } else {
            ...
            VarSymbol resultVar = new VarSymbol(FINAL|SYNTHETIC,
                                                ...
                                                arrayType,
                                                valuesSym);
            JCNewArray resultArray = make.NewArray(make.Type(types.erasure(tree.type)),null,
            ...
            JCStatement ret = make.Return(make.Ident(resultVar),0);
...
//where
    ...
    List<JCExpression> boxArgs(List<Type> parameters, List<JCExpression> _args, Type varargsElement) {
        ...
        if (varargsElement != null) {
            ...
            JCNewArray boxedArgs = make.NewArray(make.Type(varargsElement), null,
                                               List.<JCExpression>nil(),
                                               elems.toList());
            boxedArgs.type = new ArrayType(varargsElement,null,syms.arrayClass);
    ...
    JCTree boxIfNeededArgExp(JCArgExpression tree, Type type) {
        boolean havePrimitive = tree.exp1.type.isPrimitive();
        if (havePrimitive == type.isPrimitive())
            return tree;
        if (havePrimitive) {
            Type unboxedTarget = types.unboxedType(type);
            if (unboxedTarget.tag != NONE) {
                if (!types.isSubtype(tree.exp1.type, unboxedTarget))
                {
                    tree.exp1.type = unboxedTarget; // e.g. Character c = 89;
                    if(tree.exp2!=null)
                        tree.exp2.type = unboxedTarget; // e.g. Character c = 89;
                }
                tree.exp1=(JCExpression)boxPrimitive((JCExpression)tree.exp1, type);
                if(tree.exp2!=null)
                    tree.exp2=(JCExpression)boxPrimitive((JCExpression)tree.exp2, type);
                return tree;
            } else {
                tree.exp1 = (JCExpression)boxPrimitive((JCExpression)tree.exp1);
                if(tree.exp2!=null)
                    tree.exp2 = (JCExpression)boxPrimitive((JCExpression)tree.exp2);
            }
        } else {
            tree.exp1 = (JCExpression)unbox((JCExpression)tree.exp1, type);
            if(tree.exp2!=null)
                tree.exp2 = (JCExpression)unbox((JCExpression)tree.exp2, type);
        }
        return tree;
    }
    ...
    <T extends JCTree> T boxIfNeeded(T tree, Type type) {
        ...
        if(tree instanceof JCArgExpression)
            return (T)boxIfNeededArgExp((JCArgExpression)tree,type);
    ...
    public void visitReturn(JCReturn tree) {
        if (tree.expr != null&&currentMethodDef.restype!=null)
            tree.expr = translate(tree.expr,
    ...
    public void visitSelect(JCFieldAccess tree) {
        ...
        if (tree.getIdentifier() == names._class)
            result = classOf(tree.selected);
        else if (tree.getIdentifier() == names._this || tree.getIdentifier() == names._super)
 ...
 *************************************************************************/
    ...
    public List<JCTree> translateTopLevelClass(Env<AttrContext> env, JCTree cdef, TreeMaker make) {
        ...
        try {
            ...
            classdefs = new LinkedHashMap<ClassSymbol,JCClassDecl>();
            actualSymbols = new LinkedHashMap<Symbol,Symbol>();
            freevarCache = new LinkedHashMap<ClassSymbol,List<VarSymbol>>();
            proxies = new Scope(syms.noSymbol);
            outerThisStack = List.nil();
            accessNums = new LinkedHashMap<Symbol,Integer>();
            accessSyms = new LinkedHashMap<Symbol,MethodSymbol[]>();
            accessConstrs = new LinkedHashMap<Symbol,MethodSymbol>();
    ...
    private MethodSymbol addEnumFieldOrdinalMethod(JCClassDecl cdef, VarSymbol ordinalSymbol) {
        ...
        JCStatement ret = make.Return(make.Ident(ordinalSymbol),0);
    ...
    private MethodSymbol addEnumFieldNameMethod(JCClassDecl cdef, VarSymbol nameSymbol) {
        ...
        JCStatement ret = make.Return(make.Ident(nameSymbol),0);
    ...
    private MethodSymbol addEnumCompareTo(JCClassDecl cdef, VarSymbol ordinalSymbol) {
        ...
        JCReturn ret = make.Return(bin,0);
