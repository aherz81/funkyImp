...
/* ---------- parsing -------------- */
    ...
    public JCExpression qualident() {
        ...
        while (S.token() == DOT) {
            int pos = S.pos();
            S.nextToken();
            t = toP(F.at(pos).Select(t, F.at(pos).Ident(ident())));
        }
        return t;
    }
...
//where
                ...
                if ((mode & TYPE) != 0 && S.token() == LT) {
                    ...
                    if ((mode & TYPE) != 0 &&
                        ...
                        while (S.token() == DOT) {
                            S.nextToken();
                            mode = TYPE;
                            t = toP(F.at(S.pos()).Select(t, F.at(S.pos()).Ident(ident())));
            ...
            loop: while (true) {
                ...
                case LBRACKET:
                    ...
                    if (S.token() == RBRACKET) {
                        S.nextToken();
                        t = bracketsOpt(t);
                        t = toP(F.at(pos).TypeArray(t,null,false));
                        t = bracketsSuffix(t);
                    } else {
                        if ((mode & EXPR) != 0) {
                            mode = EXPR;
                ...
                case DOT:
                    ...
                    if ((mode & EXPR) != 0) {
                        ...
                        case CLASS:
                            if (typeArgs != null) return illegal();
                            mode = EXPR;
                            t = to(F.at(pos).Select(t, F.at(pos).Ident(names._class)));
                            S.nextToken();
                            break loop;
                        case THIS:
                            if (typeArgs != null) return illegal();
                            mode = EXPR;
                            t = to(F.at(pos).Select(t, F.at(pos).Ident(names._this)));
                            S.nextToken();
                            break loop;
                        case SUPER:
                            mode = EXPR;
                            t = to(F.at(pos).Select(t, F.at(pos).Ident(names._super)));
            ...
            if (S.token() == LBRACKET) {
                ...
                if ((mode & TYPE) != 0) {
                    ...
                    if (S.token() == RBRACKET) {
                        S.nextToken();
                        t = bracketsOpt(t);
                        t = toP(F.at(pos1).TypeArray(t,null,false));
                        return t;
                    }
                    mode = oldmode;
                }
                ...
                if (S.token() == SUPER && (mode & EXPR) != 0) {
                    mode = EXPR;
                    t = to(F.at(pos1).Select(t, F.at(pos1).Ident(names._super)));
                    S.nextToken();
                    t = arguments(typeArgs, t);
                    typeArgs = null;
                    ...
                    t = toP(F.at(pos1).Select(t, F.at(pos1).Ident(ident())));
    ...
    private JCArrayTypeTree bracketsOptCont(JCExpression t, int pos) {
        accept(RBRACKET);
        t = bracketsOpt(t);
        return toP(F.at(pos).TypeArray(t,null,false));
    }
    ...
    JCExpression bracketsSuffix(JCExpression t) {
        if ((mode & EXPR) != 0 && S.token() == DOT) {
            ...
            if (S.pos() == errorEndPos) {
                ...
                t = F.at(pos).Erroneous(List.<JCTree>of(toP(F.at(pos).Select(t, F.at(pos).Ident(name)))));
            } else {
                t = toP(F.at(pos).Select(t, F.at(pos).Ident(names._class)));
            }
        } else if ((mode & TYPE) != 0) {
            mode = TYPE;
        } else {
    ...
    JCExpression arrayCreatorRest(int newpos, JCExpression elemtype) {
        ...
        if (S.token() == RBRACKET) {
            ...
            return toP(F.at(newpos).NewArray(elemtype,null, dims.toList(), null));
        }
    }
    ...
    JCExpression arrayInitializer(int newpos, JCExpression t) {
        ...
        accept(RBRACE);
        return toP(F.at(newpos).NewArray(t,null, List.<JCExpression>nil(), elems.toList()));
    }
...
//todo: skip to anchor on error(?)
    ...
    public JCStatement parseStatement() {
        ...
        case RETURN: {
            ...
            JCReturn t = to(F.at(pos).Return(result,0));
            accept(SEMI);
            return t;
        }
        case THROW: {
    ...
    JCExpression annotationValue() {
        ...
        case LBRACE:
            ...
            accept(RBRACE);
            return toP(F.at(pos).NewArray(null,null, List.<JCExpression>nil(), buf.toList()));
        default:
            mode = EXPR;
            return term1();
        }
     ...
     */
    public JCTree.JCCompilationUnit parseCompilationUnit(boolean devVerbose) {
        int pos = S.pos();
    ...
    JCTree importDeclaration() {
        ...
        do {
            ...
            accept(DOT);
            if (S.token() == STAR) {
                pid = to(F.at(pos1).Select(pid, F.at(pos1).Ident(names.asterisk)));
                S.nextToken();
                break;
            } else {
                pid = toP(F.at(pos1).Select(pid, F.at(pos1).Ident(ident())));
            }
        } while (S.token() == DOT);
        accept(SEMI);
    ...
    JCVariableDecl formalParameter() {
        ...
        if (S.token() == ELLIPSIS) {
            ...
            type = to(F.at(S.pos()).TypeArray(type,null,false));
            S.nextToken();
        }
