...
public class Log extends AbstractLog {
    ...
    public static final Context.Key<PrintWriter> stdKey =
        new Context.Key<PrintWriter>();
    ...
    protected Log(Context context, PrintWriter errWriter, PrintWriter warnWriter, PrintWriter noticeWriter) {
        ...
        boolean regression = options.get("-regression") != null;
        messages = JavacMessages.instance(context);
        if(!regression)
            this.diagFormatter = rawDiagnostics ? new RawDiagnosticFormatter(options) :
                                              new BasicDiagnosticFormatter(options, messages);
        else
            this.diagFormatter = new RegressionDiagnosticFormatter(options, messages);
        ...
        if (ek != null)
            expectDiagKeys = new LinkedHashSet<String>(Arrays.asList(ek.split(", *")));
    }
    // where
    ...
    /** The std writer for diagnostics
     */
    static final PrintWriter stdWriter(Context context) {
        PrintWriter result = context.get(stdKey);
        if (result == null)
            context.put(stdKey, result = new PrintWriter(System.out));
        return result;
    }
    ...
    protected Log(Context context, PrintWriter defaultWriter) {
        this(context, defaultWriter, defaultWriter, stdWriter(context));
    }
     ...
     */
    private Set<Pair<JavaFileObject, Integer>> recorded = new LinkedHashSet<Pair<JavaFileObject,Integer>>();
     ...
     * reported so far, the diagnostic may be handed off to writeDiagnostic.
     */
    public void reportAlways(JCDiagnostic diagnostic) {
        if (expectDiagKeys != null)
            expectDiagKeys.remove(diagnostic.getCode());
