...
/** Calcs local alias information
*/

public class Alias extends TreeScanner {
    protected static final Context.Key<Alias> AliasingKey =
        new Context.Key<Alias>();
    ...
    public static Alias instance(Context context) {
        Alias instance = context.get(AliasingKey);
        if (instance == null)
            instance = new Alias(context);
        return instance;
    }

    protected Alias(Context context) {
        context.put(AliasingKey, this);
    ...
    void letInit(JCTree at,DiagnosticPosition pos, VarSymbol sym,boolean allowinout,Set<VarSymbol> s) {
        if (sym.adr >= firstadr && trackable(sym)&&!s.isEmpty())
        {
			...
			{
				VarSymbol n=i.next();
				if(n.owner!=sym.owner||(n.flags_field&Flags.IMPLICITDECL)!=0||(n.scope>=sym.scope&&(sym.flags_field&Flags.IMPLICITDECL)==0))
					i.remove();
			}
            //if(!useLinearityInfo)
			VarSymbol entry=sym;
			if(entry!=null&&!cs.isEmpty())
			{
				Set<VarSymbol> cur=sym.aliasMap.get(entry);
				if(cur==null)
				{
					sym.aliasMap.remove(entry);
					sym.aliasMap.put(entry, cs);
				}
				else
				{
					cur.addAll(cs);
					sym.aliasMap.remove(entry);
					sym.aliasMap.put(entry, cur);
				}
			}
            //else
            //    sym.aliasMapLinear.put(at, s);

            //for(Iterator<VarSymbol> i=s.)

            if((sym.flags()&FINOUT)==FOUT)
            {
                //loc_linear.excl(sym.adr);
            }
        }
 ...
 *************************************************************************/
    ...
    public void visitMethodDef(JCMethodDecl tree) {
		...
		//set addr of all member vars to -1!
...
//        nextadr = 0;
		...
		method.sym.EstimatedWork=work.analyzeTree(tree,false);
		method.sym.EstimatedMem=work.memSum;

		method.sym.EstimatedWork=work.analyzeTree(tree,true); //include inlining
		method.sym.EstimatedMem=work.memSum;
...
//          System.out.println("done [" + pass + "] : "+tree.toFlatString());
    ...
    public void visitVarDef(JCVariableDecl tree) {
        ...
        if (tree.init != null) {
            ...
            try{
                ...
                Set<VarSymbol> s=base.getAliasFromBase(sym,useLinearityInfo);
    ...
    public void visitReturn(JCReturn tree)
    {
        ...
        {
            //this is 'may'
            method.sym.retValAlias.addAll(s);
        }
    }
 ...
 *************************************************************************/
     ...
     */
    public void analyzeTree(JCTree tree, TreeMaker make,boolean lin) {
        try {
            this.make = make;
            base=new BaseSymbol();
            done=new LinkedHashSet<JCTree>();

            useLinearityInfo = lin;
