 ...
 ****************************************************************************/
    ...
    /** Find domain. Synthetic fields are always skipped.
     *  @param env     The current environment.
     ...
     */
    Symbol findDom(Env<AttrContext> env,
                     Type site,
                     Name name,
                     TypeSymbol c) {

        // get domain type of site
		DomainType dt;
		if(site instanceof ArrayType)
			dt = ((ArrayType)site).dom;
		else
			dt = (DomainType)site;

        // if names match, just return domain symbol of site
        // (parameters are already set in symbol ?)
		if(site.tsym.name.equals(name))
			return site.tsym;

        // "interfaces" are used to handle built-in selectors (reduce, size)
		if(dt.interfaces_field != null) {
			for(Type t : dt.interfaces_field)
				if(t.tsym.name.equals(name))
					return t.tsym;
		}

        // search for selected domain in environment
		Symbol sym = findIdent(env, name, DOM); // use (TYP|DOM) here???

        // selection is valid if site can be cast to the found domain
        if(sym.exists() && dt.resultDom != null && dt.resultDom.tsym == sym) {
            // parameters of result domain are not calculated here
            // since the domain symbol should not be changed
            return sym;
        }

        // selection is valid if site can be cast to the found domain
        if(sym.exists() &&sym.type.tag==TypeTags.DOMAIN&& ((DomainType)sym.type).resultDom != null && ((DomainType)sym.type).resultDom.tsym == dt.tsym) {
            // parameters of result domain are not calculated here
            // since the domain symbol should not be changed
            return sym;
        }

        if(sym.exists() &&sym.type.tag==TypeTags.DOMAIN&& ((DomainType)sym.type).parentDomain!=null&&((DomainType)sym.type).parentDomain.tsym == dt.tsym) {
            // parameters of result domain are not calculated here
            // since the domain symbol should not be changed
            return sym;
        }
    ...
    Symbol findField(Env<AttrContext> env,
        ...
        while (e.scope != null) {
            if ((e.sym.kind == VAR||(e.sym.kind==MTH&&(e.sym.flags_field&Flags.FINAL)!=0) )&& (e.sym.flags_field & SYNTHETIC) == 0) {
        ...
        if((bestSoFar instanceof VarSymbol)&&((env.enclClass.sym.flags_field&Flags.LINEAR)!=0&&(bestSoFar.owner instanceof ClassSymbol)&&bestSoFar.owner!=env.enclClass.sym))
		{
			Symbol mapped=linSymbolMap.get(bestSoFar);
			if(mapped==null)
			{
				mapped=(VarSymbol)bestSoFar.clone(bestSoFar.owner);
				mapped.type=mapped.type.addFlag(Flags.LINEAR);
				linSymbolMap.put(bestSoFar, mapped);
			}
			return mapped;
		}
		...
		if(argtypes!=null)
		{
			try {
				...
				if (rawInstantiate(env, site, sym, argtypes, typeargtypes,
					...
					case ABSENT_MTH: return new ResolveError(WRONG_MTH, syms.errSymbol, "method not found").setWrongSym(sym);
				...
				switch (bestSoFar.kind) {
				case ABSENT_MTH:
						return new ResolveError(WRONG_MTH, syms.errSymbol, "method not found").setWrongSym(sym,ex.getDiagnostic());
				case WRONG_MTH:
					return wrongMethods;
				default:
					return bestSoFar;
...
//				break;//no supertype search
        }

        if(site.tag==TypeTags.CLASS)
            site=((ClassType)site).getArrayType();

        if(site.tag==TypeTags.ARRAY)
        {
                intype=((ArrayType)site).dom;
                if(intype!=null)
                {
                        DomainSymbol c = (DomainSymbol)intype.tsym;
                        for (Scope.Entry e = c.members().lookup(name);
                                 e.scope != null;
                                 e = e.next()) {
                                //- System.out.println(" e " + e.sym);
        ...
        //disallow access to meths inherited from non-linear object inside linear object
        if(obsf!=bestSoFar&&bestSoFar.owner!=null&&(site.tsym.flags_field&Flags.LINEAR)!=0&&(bestSoFar.owner.flags_field&Flags.LINEAR)==0
				&&site.getArrayType().tag!=ARRAY)
        {
            bestSoFar = obsf;
        }
    ...
    Symbol findFun(Env<AttrContext> env, Name name,
        ...
        if (sym.exists())
        {
            if(sym.owner!=null&&(sym.owner.flags_field&Flags.LINEAR)!=0)
            {
                bestSoFar = obsf;
            }
            return sym;
        ...
        if (bestSoFar.exists())
        {
            if(bestSoFar.owner!=null&&(bestSoFar.owner.flags_field&Flags.LINEAR)!=0)
            {
                bestSoFar = obsf;
            }
            return bestSoFar;
     ...
     *  @param name      The fully qualified name of the class to be loaded.
     */
    Symbol loadDomain(Env<AttrContext> env, Name name) {
        try {
            DomainSymbol c = reader.loadDomain(name);
            return isAccessible(env, c) ? c : new AccessError(c);
        } catch (ClassReader.BadClassFile err) {
            throw err;
        } catch (CompletionFailure ex) {
     ...
     *  @param name      The type's name.
     */
    Symbol findDomain(Env<AttrContext> env, Scope scope, Name name) {
        Symbol bestSoFar = typeNotFound;
        for (Scope.Entry e = scope.lookup(name); e.scope != null; e = e.next()) {
            Symbol sym = loadDomain(env, e.sym.flatName());
            if (bestSoFar.kind == TYP && sym.kind == TYP &&
                bestSoFar != sym)
                return new AmbiguityError(bestSoFar, sym);
            else if (sym.kind < bestSoFar.kind)
    ...
    Symbol findType(Env<AttrContext> env, Name name) {
        ...
        for (Env<AttrContext> env1 = env; env1.outer != null; env1 = env1.outer) {
            ...
            JCClassDecl encl;
			if(env1.tree instanceof JCClassDecl)
				encl = env1.baseClause ? (JCClassDecl)env1.tree : env1.enclClass;
			else
				encl = env1.enclClass;
        ...
        if (env.tree.getTag() != JCTree.IMPORT) {
			...
			//ALEX: check types defined in Symtab (Object/String)..should eventually be removed
			sym = findGlobalType(env, syms.predefClass.members(), name);
            if (sym.exists()) return sym;
            else if (sym.kind < bestSoFar.kind) bestSoFar = sym;
        }
    ...
    /** Find a domain symbol.
     *  @param env       The current environment.
     *  @param name      The domain's name.
     */
    Symbol findDomain(Env<AttrContext> env, Name name) {
        Symbol bestSoFar = typeNotFound;
        Symbol sym;
        boolean staticOnly = false;
        for (Env<AttrContext> env1 = env; env1.outer != null; env1 = env1.outer) {
        ...
        if (env.tree.getTag() != JCTree.IMPORT) {
            sym = findDomain(env, env.toplevel.namedImportScope, name);
            if (sym.exists()) return sym;
            else if (sym.kind < bestSoFar.kind) bestSoFar = sym;

            sym = findDomain(env, env.toplevel.packge.members(), name);
            if (sym.exists()) return sym;
            else if (sym.kind < bestSoFar.kind) bestSoFar = sym;

            sym = findDomain(env, env.toplevel.starImportScope, name);
            if (sym.exists()) return sym;
            else if (sym.kind < bestSoFar.kind) bestSoFar = sym;

            sym = findDomain(env, syms.predefClass.members(), name);
            if (sym.exists()) return sym;
            else if (sym.kind < bestSoFar.kind) bestSoFar = sym;
/*
			sym = findField(env, syms.predefClass.type, name, syms.predefClass);
...
*/
        ...
        if ((kind & TYP) != 0) {
			...
			finally
			{
				if(bestSoFar.kind >= AMBIGUOUS)
				{
					try
					{
						Symbol sym = loadDomain(env, fullname);
						if (sym.exists()) {
							// don't allow programs to use flatnames
    ...
    Symbol findIdentInType(Env<AttrContext> env, Type site,
		...
		if ((kind & DOM) != 0 && (site instanceof ArrayType || site instanceof DomainType)) {
            findType(env, name);
            sym = findDom(env, site, name, site.tsym);
            if (sym.exists()) return sym;
            else if (sym.kind < bestSoFar.kind) bestSoFar = sym;
        }
...
//          printscopes(site.tsym.members());//DEBUG
    ...
    Symbol accessNoError(Symbol sym,
                  DiagnosticPosition pos,
                  Type site,
                  Name name,
                  boolean qualified,
 ...
 ****************************************************************************/
	...
	Map<Symbol,Symbol> linSymbolMap=new LinkedHashMap<Symbol, Symbol>();
    ...
    Symbol resolveIdent(DiagnosticPosition pos, Env<AttrContext> env,
                        Name name, int kind,VarSymbol def,int dim,JCExpression domParamTree) {
//ALEX
        Symbol sym=findIdent(env, name, kind);

		if(env.enclMethod!=null&&env.enclMethod.constraintsSyms!=null&&sym instanceof VarSymbol&&def!=null&&domParamTree!=null)
		{
			Set<Pair<VarSymbol,Pair<Integer,JCExpression>>> set=env.enclMethod.constraintsSyms.get((VarSymbol)sym);
			if(set!=null)
			{
				set.add(new Pair<VarSymbol,Pair<Integer,JCExpression>>(def,new Pair<Integer,JCExpression>(dim,domParamTree)));
				env.enclMethod.constraintsSyms.put((VarSymbol)sym, set);
			}
		}
/*
		if(sym==varNotFound&&env.enclMethod!=null)
		{
			Env<AttrContext> methEnv=env;
			while(methEnv.next!=null&&methEnv.tree!=env.enclMethod.body)
				methEnv=methEnv.next;

			for(Symbol s:env.enclMethod.implicitSyms)
			{
			...
			sym=findIdent(methEnv, name, kind);
		}
*/
        //if symbol not found, see if it's an implicit decl of shape a'b where a is defined already
        if(sym==varNotFound)
        {
            String subname[]=name.toString().split("__");

			if(subname[0].length()>0)
            {
                Name n = name.subName(0,subname[0].length());

                Symbol so = findIdent(env,n,kind);
                if(so!=varNotFound)
                {
					//clone symbol and add new var to environment
					VarSymbol sv=(VarSymbol)so;

					//declare in method body? scope!
					Env<AttrContext> methEnv=env;
					while(methEnv.next!=null&&methEnv.tree!=env.enclMethod.body)
						methEnv=methEnv.next;
					...
					VarSymbol v=sv.cloneType(name,enclScope.owner,pos.getPreferredPosition());
                    ...
                    if(so.type.isLinear()||((env.enclClass.sym.flags_field&Flags.LINEAR)!=0&&(so.owner instanceof ClassSymbol)&&so.owner!=env.enclClass.sym))
                        v.type=v.type.addFlag(Flags.LINEAR);
                    
                    if(so.type.isReadLinear())
                        v.type=v.type.addFlag(Flags.LINEARREAD);                      
                            
					if (chk.checkUnique(pos, v, enclScope)) {
						chk.checkTransparentVar(pos, v, enclScope);
						enclScope.enter(v);
					}
					env.enclMethod.implicitSyms.add(v);
					v.pos = pos.getStartPosition();
                }
				else
				{
					Symbol meth=resolveMethod(pos, env, name, null, null);
					if(meth.type.isErroneous())
						meth=varNotFound;
					...
					if(def!=null)
					{
						...
						/*
						Env<AttrContext> methEnv=;
						while(methEnv.next!=null&&methEnv.tree!=env.enclMethod.body)
							methEnv=methEnv.next;
						*/
						//Scope enclScope = env.info.scope;
						Scope enclScope = enter.enterScope(env);

						VarSymbol v=new VarSymbol(0,name,syms.intType,enclScope.owner);
						//v.flags_field|=Flags.IMPLICITDECL;
						...
						Set<Pair<VarSymbol,Pair<Integer,JCExpression>>> deps=new LinkedHashSet<Pair<VarSymbol,Pair<Integer,JCExpression>>>();
						deps.add(new Pair<VarSymbol,Pair<Integer,JCExpression>>(def,new Pair<Integer,JCExpression>(dim,domParamTree)));
						env.enclMethod.constraintsSyms.put(v,deps);
		...
		if((res instanceof VarSymbol)&&!res.type.isLinear()&&((env.enclClass.sym.flags_field&Flags.LINEAR)!=0&&(res.owner instanceof ClassSymbol)&&res.owner!=env.enclClass.sym))
		{
			Symbol mapped=linSymbolMap.get(res);
			if(mapped==null)
			{
				mapped=(VarSymbol)res.clone(res.owner);
				mapped.type=mapped.type.addFlag(Flags.LINEAR);
				linSymbolMap.put(res, mapped);
			}
			return mapped;
		}
    ...
    Symbol testQualifiedMethod(Env<AttrContext> env,
        ...
        // if no "real" method was found, it can also be a domain projection
        if(sym.kind >= AMBIGUOUS && site instanceof ArrayType) {

            // get domain type of site
            DomainType dt = ((ArrayType) site).dom;

            // search for selected domain in environment
            Symbol domsym = findIdent(env, name, DOM); // use (TYP|DOM) here???
            if(domsym.exists()) {
                // selection is valid if site can be cast to the found domain
                DomainType domtype = (DomainType) domsym.type;
                if(domtype.parentDomain != null && domtype.parentDomain.tsym == dt.tsym) {
                    // test that the correct number of argumnets is supplied
                    // test that all arguments are of type int
                    boolean nonIntType = false;
                    for(Type argtype : argtypes) {
                        if(argtype.tag != TypeTags.INT) {
                            nonIntType = true;
                            break;
                        }
                    }
                    if(!nonIntType) {
                        // parameters are not calculated here
                        // since thedomain symbol should not be changed
                        sym = domsym;
                    }
                }
            }
        ...
        if (sym.kind >= AMBIGUOUS) {//if nothing is found return the 'first' error
			...
			Symbol nsym = findIdentInType(env, site, name, DOM);
			if (nsym.kind < AMBIGUOUS)
			{
				return nsym;
			}
		}
    ...
    Symbol findQualifiedMethod(DiagnosticPosition pos, Env<AttrContext> env,
                                  Type site, Name name, List<Type> argtypes,
                                  List<Type> typeargtypes,boolean allowDomain) {
        Symbol sym = methodNotFound;
        ...
        // if no "real" method was found, it can also be a domain projection
        if(allowDomain&&sym.kind >= AMBIGUOUS) {
            ...
            if(site.tag==TypeTags.ARRAY)
            {
                ...
                if(domsym.exists()) {
                    ...
                    if(domtype.parentDomain != null && domtype.parentDomain.tsym == dt.tsym) {
                        // test that the correct number of argumnets is supplied
                        if(argtypes.length() != domtype.projectionArgs.length()) {
                            log.error(pos, "domain.wrong.number.of.indices", domsym);
                        }
                        // test that all arguments are of type int
                        boolean nonIntType = false;
                        for(Type argtype : argtypes) {
                            if(argtype.tag != TypeTags.INT) {
                                log.error(pos, "domain.projecion.used.with.non.int", domsym);
                                nonIntType = true;
                                break;
                            }
                        }
        ...
        if (allowDomain&&sym.kind >= AMBIGUOUS) {//if nothing is found return the 'first' error
                        if(site.tag == ARRAY)
				site = ((ArrayType)site).dom;
			// We are seeing a plain identifier as selector.
			Symbol nsym = findIdentInType(env, site, name, DOM);
			if (nsym.kind < AMBIGUOUS)
			{
				nsym = access(nsym, pos, site, name, true);
				return nsym;
			}
		}
		...
		if (sym.kind >= AMBIGUOUS) {
			MethodResolutionPhase errPhase =
                    firstErroneousResolutionPhase();
			sym = accessNoError(methodResolutionCache.get(errPhase),
    ...
    Symbol resolveQualifiedMethod(DiagnosticPosition pos, Env<AttrContext> env,
                                  Type site, Name name, List<Type> argtypes,
                                  List<Type> typeargtypes) {
        Symbol sym = findQualifiedMethod(pos,env,site,name,argtypes,typeargtypes,true);

        if (sym.kind >= AMBIGUOUS||sym.kind==ERR) {//if nothing is found return the 'first' error
            MethodResolutionPhase errPhase =
                    firstErroneousResolutionPhase();
            sym = access(methodResolutionCache.get(errPhase),
                    pos, site, name, true, argtypes, typeargtypes);
     ...
     *  @param right     The types of the right operand.
     */
    Symbol resolveDomain(DiagnosticPosition pos,
                                 Name name,
                                 Env<AttrContext> env
                                 ) {
        Symbol sym=findIdent(env, name, TYP|DOM);
 ...
 ****************************************************************************/
    ...
    private Map<MethodResolutionPhase, Symbol> methodResolutionCache =
        new LinkedHashMap<MethodResolutionPhase, Symbol>(MethodResolutionPhase.values().length);
    ...
    private MethodResolutionPhase firstErroneousResolutionPhase() {
		...
		if(methodResolutionCache.get(BASIC)!=null)
			return BASIC;
		if(methodResolutionCache.get(BOX)!=null)
			return BOX;
		if(methodResolutionCache.get(VARARITY)!=null)
			return VARARITY;
