 ...
 ***************************************************************************/
     ...
     */
    LookupMapping fromTypeVarFun = new LookupMapping("fromTypeVarFun") {
            public Type apply(Type t,java.util.Map<String,UndetVar> map) {
                if (t.tag == TYPEVAR)
				{
					UndetVar uv=map.get(t.toString());
					if(uv==null)
					{
						uv=new UndetVar(t);
						map.put(t.toString(),uv);
					}
					((TypeVar)uv.qtype).bound=apply(uv.qtype.getSafeUpperBound(),map);
					return uv;
				}
				else if(t.tag == ARRAY) return new ArrayType(apply(((ArrayType)t).elemtype,map),((ArrayType)t).dom,syms.arrayClass);
 ...
 ***************************************************************************/
    ...
    public Type instantiateExpr(ForAll that,
        ...
        List<Type> undetvars = Type.map(that.tvars, fromTypeVarFun,new java.util.LinkedHashMap<String,UndetVar>());
        for (List<Type> l = undetvars; l.nonEmpty(); l = l.tail) {
			if(l.head.tag==TypeTags.UNDETVAR)
			{
    ...
    public Type instantiateMethod(List<Type> tvars,
        ...
        List<Type> undetvars = Type.map(tvars, fromTypeVarFun,new java.util.LinkedHashMap<String,UndetVar>());
        ...
        Type res= types.subst(mt, tvars, insttypes.toList());
		return res;
    }
    //where
