...
public class RecognizedOptions {
    ...
    static Set<OptionName> javacOptions = EnumSet.of(G,
        G_NONE,
        G_CUSTOM,
        XLINT,
        XLINT_CUSTOM,
    ...
    public static Option[] getAll(final OptionHelper helper) {
        ...
        new Option(BARVINOK,                                     "opt.barvinok"),
        new Option(DEPGRAPH,                                     "opt.depgraph"),
        new Option(USEOPENCL,                                     "opt.useopencl"),
        new Option(SIMPLEMEMMODEL,				"opt.simplememmodel"),
	new Option(CPP,                                         "opt.cpp"),
        new Option(MPI,                                         "opt.mpi"),
        new Option(VERIFYARRAYS,                                "verify.arrays"),
        new Option(RTVERIFYARRAYS,                                "run.time.verify.arrays"),

        new Option(OPTIMIZERECURSION,                           "opt.optimizerecursion"),
        new Option(LLVM,                                        "opt.llvm"),
        new Option(ANALYSIS,                                     "opt.analysis"),
        new Option(NOPATHMERGE,                                  "opt.no.path.merge"),

        new Option(FORCEGCCLEANUP,                              "opt.force.gc"),
        new Option(DUMPMERGE,									"opt.dump.merge"),
        new Option(NOPARALLELPATHS,                              "opt.no.parallel.paths"),
        new Option(REGRESSION,                                  "opt.regression"),
        new Option(SKIPGENERATE,                                 "opt.skip.generate"),
        ...
        new Option(USELOCALHACK,           "opt.arg.path",      "opt.localhack"),
        new Option(PROFILE,                "opt.arg.path",      "opt.profile"),
        ...
        new Option(PP,                     "opt.arg.dirs",      "opt.pps"),
        new Option(CONFIG,                 "opt.arg.dirs",      "opt.config"),
        new Option(WORK,					"opt.arg.dirs",           "opt.work"),
        new Option(PROFILE,                "opt.profile"),
        new Option(CC,                     "javac.opt.arg.args",      "opt.cc"),
        ...
        new HiddenOption(SOURCEFILE) {
            ...
            public boolean matches(String s) {
                this.s = s;
                return s.endsWith(".java")||s.endsWith(".funky")||s.endsWith(".f")  // Java source file
                    || SourceVersion.isName(s);   // Legal type name
            }
            @Override
            public boolean process(Options options, String option) {
                if (s.endsWith(".java")||s.endsWith(".funky")||s.endsWith(".f") ) {
                    File f = new File(s);
                    if (!f.exists()) {
