...
public class Types {
    ...
    // where
        private final SimpleVisitor<Type,Symbol> asSub = new SimpleVisitor<Type,Symbol>() {
            ...
            @Override
            public Type visitDomainErrorType(DomainErrorType t, Symbol sym) {
                return t;
            }
        };
    // </editor-fold>
    ...
    public boolean isSubtypeUnchecked(Type t, Type s, Warner warn) {
        //transparently substutite class x extends array by array
        if(t.tag==CLASS&&((ClassType)t).getArrayType().tag==ARRAY) //FIXME: if true: copy uniqueness
        {
            return isSubtype(t, s)|isSubtypeUnchecked(((ClassType)t).getArrayType(),s,warn);
        }
        if(s.tag==CLASS&&((ClassType)s).getArrayType().tag==ARRAY)
        {
            return isSubtype(t, s)|isSubtypeUnchecked(t,((ClassType)s).getArrayType(),warn);
        }

       if (t.tag == ARRAY || s.tag == ARRAY) {
            if(t.tsym==null&&s.tsym==null)
                    return true;

            if(t.tag == UNDETVAR)
            {
                    ((UndetVar)t).lobounds = ((UndetVar)t).lobounds.prepend(s);
                    t=((UndetVar)t).qtype.getSafeUpperBound();
            }

            if(s.tag == UNDETVAR)
            {
                    ((UndetVar)s).lobounds = ((UndetVar)s).lobounds.prepend(t);
                    s=((UndetVar)s).qtype.getSafeUpperBound();
            }

            if (t.tag != ARRAY || s.tag != ARRAY)
                    return isSubtype(t, s);

            if(t.tsym==null&&((ArrayType)s).dom!=null)
                    return true;
            if(((ArrayType)t).dom!=null&&s.tsym==null)
                    return true;

            return isSubtypeUncheckedArrays((ArrayType)t, (ArrayType)s, warn);
	...
	boolean isDynamic(List<JCTree.JCExpression> params)
	{
		for(JCTree.JCExpression e:params)
		{
			if(e.type.constValue()==null||!(e.type.constValue() instanceof Integer))
				return true;
    ...
    /**
     * Is array type t1 a subtype of array type t2?
     */
	static boolean isSameParams(List<JCTree.JCExpression> params1,List<JCTree.JCExpression> params2)
	{
		if(params1==null||params2==null)
			return false;

        boolean sameParams = params1.size()==params2.size();

		if(sameParams)
		{
			Iterator<JCTree.JCExpression> i1=params1.iterator();
			Iterator<JCTree.JCExpression> i2=params2.iterator();

			while(i1.hasNext())
			{
				JCTree.JCExpression p1=i1.next();
				JCTree.JCExpression p2=i2.next();

				Type s=p2.type;
				if(s.tag == UNDETVAR)
				{
					Type tc=p1.type;
					if(p1.getTag()==JCTree.IDENT)
						tc=p1.type.constType(p1);
					((UndetVar)s).lobounds = ((UndetVar)s).lobounds.prepend(tc);
				}
				...
				{
					Type tc=p1.type;
					if(p2.getTag()==JCTree.IDENT)
						tc=p1.type.constType(p1);
					((UndetVar)t).lobounds = ((UndetVar)t).lobounds.prepend(tc);
				}

				Object o1=p1.type.constValue();
				Object o2=p2.type.constValue();

				if(o1==null||o2==null)
				{
					if(!(o1==null&&o2==null))
					{
						if(!(o1==null&&(o2.equals(-1)||t.tag==UNDETVAR))&&!(o2==null&&(o1.equals(-1)||s.tag==UNDETVAR)))
						{
							sameParams=false;
						//	break;
						}
    ...
    public boolean isSubtypeUncheckedArrays(ArrayType t1, ArrayType t2, Warner warn) {
        // two things have to be compared: the element type and the domain

        // 1. check element types
        //boolean isPrimitive = (t1.elemtype.tag <= lastBaseTag);
        boolean compatibleElementTypes = isSubtypeUnchecked(elemtype(t1), elemtype(t2), warn);

		if(elemtype(t1).isPointer()!=elemtype(t2).isPointer())
			return false;

        if(!compatibleElementTypes) return false;

		Type s=elemtype(t2);
		if(s.tag == UNDETVAR)
		{
				((UndetVar)s).lobounds = ((UndetVar)s).lobounds.prepend(elemtype(t1));
		}
		Type t=elemtype(t1);
		if(t.tag == UNDETVAR)
		{
				((UndetVar)s).lobounds = ((UndetVar)s).lobounds.prepend(elemtype(t2));
		}

		if(t1.dom==null&&t2.dom==null)
			return true;
        ...
        // 2. check domains
        // 2.1 test if domais have the same type
        boolean sameDomain = (t1.dom.tsym == t2.dom.tsym);
        if(!sameDomain)
			return isConvertible(t2.dom, t1.dom);
					//||isSameType(t1.dom,t2.dom.resultDom)||isSameType(t1.dom,t2.dom.parentDomain);

        // 2.2 test if domians have the same parameters
        List<JCTree.JCExpression> params1 = t1.dom.appliedParams;
        List<JCTree.JCExpression> params2 = t2.dom.appliedParams;
        boolean sameParams=isSameParams(params1,params2);

		//if dynamic then we check at runtime
		if(isDynamic(params2))
			return true;
    ...
    final public boolean isSubtype(Type t, Type s) {
		//Alex: everything is a subtype of bot:
		//FIXME: java's type system can handle only classes
		/*
		 *
    ...
    public boolean isSubtype(Type t, Type s, boolean capture) {
        ...
        if (t.tsym.name.equals(s.tsym.name)&&t.isPrimitive())
            return true;

        if(s.tag!=UNDETVAR&&s.getSafeUpperBound().tsym.name.equals(names.Object))
                return true;
        ...
        private TypeRelation isSubtype = new TypeRelation()
        {
            public Boolean visitType(Type t, Type s) {
				...
				case DOMAIN:
					if(s.tag!=DOMAIN)
						return false;
					DomainType dt=(DomainType)t;
					DomainType ds=(DomainType)s;

					if(isSameType(dt,ds))
						if(isSameParams(dt.appliedParams,ds.appliedParams))
							return true;

					if(isSameType(dt.parentDomain,ds))
						if(isSameParams(dt.getParentParams(),ds.appliedParams))
							return true;

					if(isSameType(dt.resultDom,ds))
						if(isSameParams(dt.getResultParams(),ds.appliedParams))
							return true;

					/*
					if(isSameType(ds.parentDomain,dt))
						if(isSameParams(ds.getParentParams(),dt.appliedParams))
							return true;
					*/

					if(isSameType(ds.resultDom,dt))
						if(isSameParams(ds.getResultParams(),dt.appliedParams))
							return true;

					//return dt.canCastTo(null, null, dt.appliedParams.toArray(new Integer[0]), ds, ds.appliedParams.toArray(new Integer[0]));
					return false;
            ...
            private Set<TypePair> cache = new LinkedHashSet<TypePair>();
            ...
            public Boolean visitArrayType(ArrayType t, Type s) {
                if (s.tag == ARRAY) {
                    if(!isSubtype(t.dom,((ArrayType)s).dom))
                        return false;
                ...
                if (s.tag == CLASS) {
					Type st=((ClassType)s).supertype_field;

					if(st!=null&&st.tag==TypeTags.ARRAY)
						return visitArrayType(t,st);
    ...
    public boolean isSuperType(Type t, Type s) {
        ...
        case UNDETVAR: {
            ...
            if(isSuperType(undet.qtype.getUpperBound(),s))
            {
                undet.lobounds = undet.lobounds.prepend(s);
                return true;
            }
            else if(isSuperType(undet.qtype.getUpperBound(),s.getArrayType()))
            {
                undet.lobounds = undet.lobounds.prepend(s);
                return true;
            }
        ...
        private TypeRelation isSameType = new TypeRelation() {
            ...
            public Boolean visitClassType(ClassType t, Type s) {
                ...
                if (t.isCompound() && s.isCompound()) {
                    ...
                    LinkedHashSet<SingletonType> set = new LinkedHashSet<SingletonType>();
            ...
            public Boolean visitArrayType(ArrayType t, Type s) {
                ...
                return s.tag == ARRAY
                    && containsTypeEquivalent(t.elemtype, elemtype(s))
						&&visit(t.dom,((ArrayType)s).dom);
            }

            @Override
            public Boolean visitDomainType(DomainType t, Type s) {
                if (t == s)
                    return true;
                ...
                return s.tag == DOMAIN //FIXME: check that params match?
                    && ((DomainType)s).tsym.name.equals(t.tsym.name);
            }
    ...
    public boolean isCastable(Type t, Type s, Warner warn) {
		if(s.isLinear()&&!t.isLinear()&&t!=syms.botType&&!t.isReadLinear())
			return false;
    ...
    // where
        private TypeRelation isCastable = new TypeRelation() {
            ...
            public Boolean visitType(Type t, Type s) {
                ...
                switch (t.tag) {
                case BYTE: case CHAR: case SHORT: case INT: case LONG: case FLOAT: case BOOLEAN:
                case DOUBLE:
            ...
            @Override
            public Boolean visitDomainType(DomainType t, Type s) {
				if(s.tag!=TypeTags.DOMAIN)
					return false;
				if(t.isBaseDomain!=((DomainType)s).isBaseDomain)
					return false;
				return t.canCastTo(((DomainSymbol)t.tsym).declarationPos,t.appliedParams.toArray(new JCTree.JCExpression[0]), (DomainType)s, ((DomainType)s).appliedParams.toArray(new JCTree.JCExpression[0]));
			}
            ...
            public Boolean visitArrayType(ArrayType t, Type s) {
                ...
                case CLASS:
                    ...
                    return sname == names.java_lang_Object
                        ...
                        || sname == names.java_lang_Cloneable
                        || sname == names.java_io_Serializable
                        || (t.elemtype.tag==TypeTags.CHAR&&sname.equals(names.String));


                case ARRAY:
                    if(((ArrayType)s).dom!=null&&t.dom!=null&&!isCastable(t.dom,((ArrayType)s).dom))
                        return false;
                    if (t.elemtype.tag <= lastBaseTag)
                        return isSameType(t.elemtype, elemtype(s));
                    else
	...
	public Type domtype(Type t) {
        switch (t.tag) {
        case WILDCARD:
            return null;
        case ARRAY:
            return ((ArrayType)t).dom;
        case FORALL:
            return null;
        case ERROR:
            return t;
    ...
    public boolean isAssignable(Type t, Type s, Warner warn) {
        if (t.tag == ERROR)
            return false;
		if(!s.isLinear()&&!s.isReadLinear()&&t.isReadLinear())
			return false;
    ...
    public Type erasure(Type t) {
        Type out= erasure(t, false);
		out.type_flags_field|=t.type_flags_field; //retain flags
		return out;
    }
    //where
        ...
        private UnaryVisitor<Type> supertype = new UnaryVisitor<Type>() {
            ...
            public Type visitArrayType(ArrayType t, Void ignored) {
                if (t.elemtype.isPrimitive() || isSameType(t.elemtype, syms.objectType))
                    ...
                    return new ArrayType(supertype(t.elemtype),t.dom,t.tsym);
            }
    ...
    Map<Type,Boolean> isDerivedRawCache = new LinkedHashMap<Type,Boolean>();
    ...
    public boolean overrideEquivalent(Type t, Type s) {
        return hasSameArgs(t, s) ||
            hasSameArgs(t, s) || hasSameArgs(t, s);
    }
    ...
    private class Subst extends UnaryVisitor<Type> {
        ...
        public Type visitTypeVar(TypeVar t, Void ignored) {
            for (List<Type> from = this.from, to = this.to;
				...
				//ALEX: cannot use t==from.head, some copy process might clone t
				if (t.tsym.name.equals(from.head.tsym.name)) {
        ...
        public Type visitArrayType(ArrayType t, Void ignored) {
			...
			{
				DomainType d=t.dom;
				if(d!=null&&d.appliedParams!=null)
				{
					d=(DomainType)d.clone();
					ListBuffer<JCTree.JCExpression> app=new ListBuffer<JCTree.JCExpression>();

					for(JCTree.JCExpression e:d.appliedParams)
					{
						//JavaCompiler.getCompiler().
						e=(JCTree.JCExpression)e.clone();
						e.type=subst(e.type);
						app.add(e);
					}
					d.appliedParams=app.toList();
				}
                return new ArrayType(upperBound(elemtype),d,t.tsym);
			}
        }
    ...
    public Type createErrorTypeDom(Name name, TypeSymbol container, Type originalType) {
        return new DomainErrorType(name, container, originalType);
    }
    // </editor-fold>
    ...
    public int rank(Type t) {
        ...
        case CLASS: {
            ...
            if (cls.rank_field < 0) {
                ...
                if (fullname == names.java_lang_Object||fullname == names.Object)
                    cls.rank_field = 0;
                else {
                    int r;
					if(supertype(cls).tag==CLASS)
						r= rank(supertype(cls));
					else
						r= 0;
     ...
     * otherwise).
     */
    private Map<Type,List<Type>> closureCache = new LinkedHashMap<Type,List<Type>>();
    ...
    // where
        ...
        Set<TypePair> mergeCache = new LinkedHashSet<TypePair>();
    ...
    public Type lub(List<Type> ts) {
        ...
        case ARRAY_BOUND:
            ...
            // lub(A[], B[]) is lub(A, B)[]
			//FIXME: domains??
            return new ArrayType(lub(elements),null,syms.arrayClass);
        ...
        default:
			if(primitive!=null)
				return syms.errType;
            // calculate lub(A, B[])
            List<Type> classes = (new ListBuffer<Type>()).toList();
            for (Type t : ts) {
    ...
    class Adapter extends SimpleVisitor<Void, Type> {
        ...
        Adapter(ListBuffer<Type> from, ListBuffer<Type> to) {
            this.from = from;
            this.to = to;
            mapping = new LinkedHashMap<Symbol,Type>();
        }
    ...
    public static abstract class DefaultTypeVisitor<R,S> implements Type.Visitor<R,S> {
        ...
        public R visitDomainType(DomainType t, S s)       { return visitType(t, s); }
        public R visitDomainErrorType(DomainErrorType t, S s) { return visitType(t, s); }
    }
