...
public class TreeInfo {
    ...
    public static int getStartPos(JCTree tree) {
        ...
        switch(tree.getTag()) {
//        case(JCTree.PRAGMA):
//            return getStartPos(((JCPragma) tree).cond);
        case(JCTree.APPLY):
...
//ALEX
            public void visitDomainDef(JCDomainDecl that) {
                if (that.sym == sym) result = that;
                else super.visitDomainDef(that);
            }
    ...
    public static Name name(JCTree tree) {
        ...
        case JCTree.SELECT:
            return ((JCFieldAccess) tree).getIdentifier();
        case JCTree.TYPEAPPLY:
     ...
     *  otherwise return null.
     */
    public static Symbol symbolRead(JCTree tree) {
        tree = skipParens(tree);
        switch (tree.getTag()) {
        case JCTree.IDENT:
            return ((JCIdent) tree).sym;
        case JCTree.DOMITER:
            return ((JCDomainIter) tree).sym;
        case JCTree.ARGEXPRESSION:
			if(((JCArgExpression) tree).exp2!=null)
				return symbol(((JCArgExpression) tree).exp2);
            return null;
        case JCTree.SELECT:
    ...
    public static Symbol symbol(JCTree tree) {
        ...
        case JCTree.DOMITER:
            return ((JCDomainIter) tree).sym;
        case JCTree.WHERE:
            return symbol(((JCWhere) tree).exp);
        case JCTree.ARGEXPRESSION:
            return symbol(((JCArgExpression) tree).exp1);
        case JCTree.SELECT:
            return ((JCFieldAccess) tree).sym;
        case JCTree.TYPEAPPLY:
            return symbol(((JCTypeApply) tree).clazz);
    ...
    public static Symbol rootSymbol(JCTree tree) {
        if(tree==null)
            return null;
        tree = skipParens(tree);
        switch (tree.getTag()) {
        case JCTree.IDENT:
            return ((JCIdent) tree).sym;
        case JCTree.INDEXED:
            return rootSymbol(((JCArrayAccess) tree).indexed);
        case JCTree.DOMITER:
            return ((JCDomainIter) tree).sym;
        case JCTree.WHERE:
            return rootSymbol(((JCWhere) tree).exp);
        case JCTree.ARGEXPRESSION:
            return rootSymbol(((JCArgExpression) tree).exp1);
        case JCTree.SELECT:
            return rootSymbol(((JCFieldAccess) tree).selected);
        case JCTree.TYPEAPPLY:
            return rootSymbol(((JCTypeApply) tree).clazz);
        case JCTree.APPLY:
            return rootSymbol(((JCMethodInvocation) tree).meth);
		case JCTree.VARDEF:
			return ((JCVariableDecl)tree).sym;
        default:
            return null;
    ...
    public static void setsymbol(JCTree tree,Symbol s) {
        if(tree==null)
            return;
        tree = skipParens(tree);
        switch (tree.getTag()) {
        case JCTree.IDENT:
			...
			return;
        case JCTree.DOMITER:
            ((JCDomainIter) tree).sym=s;
			return;
        case JCTree.ARGEXPRESSION:
            setsymbol(((JCArgExpression) tree).exp1,s);
			return;
        case JCTree.SELECT:
            ((JCFieldAccess) tree).sym=s;
			return;
        case JCTree.TYPEAPPLY:
            setsymbol(((JCTypeApply) tree).clazz,s);
			return;
        case JCTree.APPLY:
            setsymbol(((JCMethodInvocation) tree).meth,s);
			return;
		case JCTree.VARDEF:
			((JCVariableDecl)tree).sym=(Symbol.VarSymbol)s;
			return;
        default:
            return;
        }
    ...
    public static String flagNames(long flags) {
        return Flags.toString(flags & (StandardFlags|FINOUT|LINEAR|UNSIGNED)).trim();
    }
    ...
    public static int opPrec(int op) {
        ...
        case JCTree.SEQ: return shiftPrec;
        case JCTree.PLUS:
