...
public class TreeMaker implements JCTree.Factory {
    ...
    public JCClassDecl SingularDef(JCModifiers mods,
                                Name name,
                                List<JCTypeParameter> typarams,
                                JCTree extending,
                                List<JCExpression> implementing,
	...
	public JCJoinDomains Join(List<JCExpression> doms)
	{
		JCJoinDomains tree=new JCJoinDomains(doms);
		tree.pos=pos;
		return tree;
	}
...
//ALEX
   public JCArgExpression ArgExpression( JCExpression exp1,
                          JCExpression exp2)
   {
          JCArgExpression tree = new JCArgExpression(exp1,
                                     exp2);
            tree.pos = pos;
            return tree;
   }

    public JCDomParameter DomParameter( Name name )
    {
          JCDomParameter tree = new JCDomParameter(
                                     name);
            tree.pos = pos;
            return tree;
    }

    public JCCTProperty CTP( JCExpression exp, Name name )
    {
          JCCTProperty tree = new JCCTProperty(exp,name);
            tree.pos = pos;
            return tree;
    }

    public JCDomInstance DomInstance( Name name, List<JCExpression> domparams )
    {
          JCDomInstance tree = new JCDomInstance(
                                     name,domparams);
            tree.pos = pos;
            return tree;
    }

    public JCDomParent DomParent( Name name, List<JCExpression> domparams,List<JCDomParameter> domargs )
    {
          JCDomParent tree = new JCDomParent(
                                     name,domparams,domargs);
            tree.pos = pos;
            return tree;
    }

    public JCDomUsage DomUsage( Name name, List<JCDomParameter> domparams )
    {
          JCDomUsage tree = new JCDomUsage(
                                     name,domparams);
            tree.pos = pos;
            return tree;
    }

    public JCDomConstraintValue ConstraintValue(int sign, JCExpression literal,
            JCDomParameter dom_param)
    {
          JCDomConstraintValue tree = new JCDomConstraintValue(
                                     sign,
                                     literal,
                                     dom_param);
            tree.pos = pos;
    ...
    public JCDomConstraintValue ConstraintValue(int sign, JCDomParameter param,
            JCDomParameter dom_param)
    {
          JCDomConstraintValue tree = new JCDomConstraintValue(
                                     sign,
    ...
    public JCDomConstraint ConstrainDef(List<JCDomConstraintValue> left, int assign,
            List<JCDomConstraintValue> right)
    {
          JCDomConstraint tree = new JCDomConstraint(
                                     left,
                                     assign,
                                     right);
            tree.pos = pos;
    ...
    public JCDomainDecl DomDef( Name name,
						  ...
						  JCDomParent parent)
        {
          JCDomainDecl tree = new JCDomainDecl(
                                     name,
                                     domparams,
                                     domargs,
                                     //implementing,
...
//END
    ...
    public JCCF NewCF(JCTree condition,boolean value,int pos)
    {
        JCCF cf = new JCCF(condition,value);
        cf.pos = pos;
        return cf;
    }


    public JCMethodDecl EventDef(JCModifiers mods,
                               Name name,
                               //JCExpression restype,
                               List<JCTypeParameter> typarams,
                               List<JCVariableDecl> params,
	...
	public JCVariableDecl VarDef(JCModifiers mods, Name name, JCExpression vartype, JCExpression init,com.sun.tools.javac.util.List<JCVariableDecl> list) {
        JCVariableDecl tree = new JCVariableDecl(mods, name, vartype, init, null,list);
        tree.pos = pos;
        return tree;
    }
    ...
    public JCIfExp IfExp(JCExpression cond, JCExpression thenpart, JCExpression elsepart)
    {
        JCIfExp tree = new JCIfExp(cond, thenpart, elsepart);
        tree.pos = pos;
        return tree;
    }
    ...
    public JCReturn Return(JCExpression expr,long flags) {
        JCReturn tree = new JCReturn(expr,flags);
        tree.pos = pos;
        return tree;
    }
    ...
    public JCPragma Pragma(long type,JCExpression cond, JCExpression detail) {
        JCPragma tree = new JCPragma(type,cond, detail);
        tree.pos = pos;
        return tree;
    }
    ...
    {
        JCNewArray tree = new JCNewArray(elemtype,dom, dims, elems);
        tree.pos = pos;
        return tree;
    }
    ...
    public JCAssign Assign(JCExpression lhs, JCExpression rhs,long aflags,JCExpression cond) {
        JCAssign tree = new JCAssign(lhs, rhs, aflags, cond);
        tree.pos = pos;
        return tree;
    }

    public JCAssign Assign(JCExpression lhs, JCExpression rhs) {
        JCAssign tree = new JCAssign(lhs, rhs, 0, null);
        tree.pos = pos;
        return tree;
    }

    public JCWhere Where(JCExpression exp,JCStatement stmnt,JCExpression sexp) {
        JCWhere tree = new JCWhere(exp,stmnt,sexp);
        tree.pos = pos;
        return tree;
    }

    public JCFor StaticFor(Name name, JCExpression exp, List<JCTree> content) {
        JCFor tree = new JCFor(name,exp,content);
        tree.pos = pos;
        return tree;
    }

    public JCSelect Select(List<JCSelectCond> list) {
        JCSelect tree = new JCSelect(list);
        tree.pos = pos;
        return tree;
    }

    public JCCaseExp CaseExp(JCExpression exp,List<JCSelectCond> list) {
        JCCaseExp tree = new JCCaseExp(exp,list);
        tree.pos = pos;
        return tree;
    }

    public JCSelectCond SelectCond(JCExpression cond,JCExpression res,JCStatement stmnt) {
        JCSelectCond tree = new JCSelectCond(cond,res,stmnt);
        tree.pos = pos;
        return tree;
    }

    public JCDomainIter DomIter(JCExpression exp,Name name,List<JCVariableDecl> domargs,JCExpression sexp,JCStatement sbody, List<JCExpression> params) {
        JCDomainIter tree = new JCDomainIter(exp,name,domargs,sexp,sbody,params);
        tree.pos = pos;
        return tree;
  }
    ...
    public JCSequence Sequence(List<JCExpression> seq) {
        JCSequence tree = new JCSequence(seq);
        tree.pos = pos;
        return tree;
    }
    ...
    public JCArrayAccess Indexed(JCExpression indexed, List<JCExpression> index, List<JCExpression> params) {
        JCArrayAccess tree = new JCArrayAccess(indexed, index, params);
        tree.pos = pos;
        return tree;
    }

	public JCArrayAccess Indexed(JCExpression indexed, List<JCExpression> index) {
        JCArrayAccess tree = new JCArrayAccess(indexed, index, null);
        tree.pos = pos;
        return tree;
    }

    public JCFieldAccess Select(JCExpression selected, JCExpression selector) {
        JCFieldAccess tree = new JCFieldAccess(selected, selector, null,null);
        tree.pos = pos;
        return tree;
    }

	public JCFieldAccess Select(JCExpression selected, JCExpression selector, List<JCExpression> params) {
        JCFieldAccess tree = new JCFieldAccess(selected, selector, null,params);
        tree.pos = pos;
        return tree;
    }
	...
	public JCSizeOf SizeOf(JCExpression e) {
        JCSizeOf tree = new JCSizeOf(e);
        tree.pos = pos;
        return tree;
    }
    ...
    public JCArrayTypeTree TypeArray(JCExpression elemtype,JCDomInstance dom,boolean option) {
        JCArrayTypeTree tree = new JCArrayTypeTree(elemtype,dom,option);
        tree.pos = pos;
        return tree;
    }
    ...
    public JCModifiers Modifiers(long flags, List<JCAnnotation> annotations, JCExpression group, JCExpression thread, JCExpression work, JCExpression task,JCExpression mem) {
        JCModifiers tree = new JCModifiers(flags, annotations,group,thread,work,task,mem);
    ...
    public JCModifiers Modifiers(long flags, List<JCAnnotation> annotations) {
        JCModifiers tree = new JCModifiers(flags, annotations,null);
        boolean noFlags = (flags & Flags.StandardFlags) == 0;
        tree.pos = (noFlags && annotations.isEmpty()) ? Position.NOPOS : pos;
        return tree;
    }
 ...
 ****************************************************************************/
    ...
    public JCExpression Select(JCExpression base, Symbol sym) {
        return new JCFieldAccess(base, Ident(sym.name), sym).setPos(pos).setType(sym.type);
    }
    ...
    public JCExpression QualIdent(Symbol sym) {
        return (isUnqualifiable(sym)||sym.owner.kind==Kinds.PCK)
            ? Ident(sym)
    ...
    public JCExpression Type(Type t) {
        ...
        case CLASS:
            ...
            break;
        case ARRAY:
			JCDomInstance di=null;
			DomainType dt=((ArrayType)t).dom;
			if(dt!=null)
			{
				ListBuffer<JCExpression> params=new ListBuffer<JCExpression>();

				for(JCExpression i:dt.appliedParams)
					params.add(i);

				di=DomInstance(dt.tsym.name,params.toList());
			}
            tp = TypeArray(Type(types.elemtype(t)),di,types.elemtype(t).isPointer());
            break;
        case ERROR:
            tp = TypeIdent(ERROR);
            break;
...
//where
    ...
    class AnnotationBuilder implements Attribute.Visitor {
        ...
        public JCAnnotation visitCompoundInternal(Attribute.Compound compound) {
            ...
            for (List<Pair<Symbol.MethodSymbol,Attribute>> values = compound.values; values.nonEmpty(); values=values.tail) {
                ...
                args.append(Assign(Ident(pair.fst), valueTree,0,null).setType(valueTree.type));
            }
        ...
        public void visitArray(Attribute.Array array) {
            ...
            result = NewArray(null,null, List.<JCExpression>nil(), elems.toList()).setType(array.type);
        }
    ...
    public JCStatement Call(JCExpression apply) {
        return apply.type.tag == VOID ? Exec(apply) : Return(apply,0);
    }
    ...
    public JCStatement Assignment(Symbol v, JCExpression rhs) {
        return Exec(Assign(Ident(v), rhs,0,null).setType(v.type));
    }
    ...
    public JCArrayAccess Indexed(Symbol v, JCExpression index) {
		...
		lb.add(index);
        JCArrayAccess tree = new JCArrayAccess(QualIdent(v), lb.toList());
