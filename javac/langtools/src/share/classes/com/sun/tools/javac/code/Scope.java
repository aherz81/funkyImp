...
public class Scope {
    ...
    Entry makeEntry(Symbol sym, Entry shadowed, Entry sibling, Scope scope, Scope origin) {
        return Entry.addEntry(sym, shadowed, sibling, scope);
    }
    ...
    public static class Entry {
		...
		static long countDepth(Scope scope)
		{
			if(scope==null)
				return 0;
			long c=0;
        ...
        public Entry(Symbol sym, Entry shadowed, Entry sibling, Scope scope) {
			this.sym = sym;
			this.shadowed = shadowed; //prev table entry (higher scope)
			this.sibling = sibling; //rest of SLL (increasing scope)
			this.scope = scope;
		}

        public static Entry addEntry(Symbol sym, Entry shadowed, Entry sibling, Scope scope) {

			long depth=countDepth(scope);
			Entry end=shadowed;
			Entry prev=null;

			if(end!=null)
			while(depth<countDepth(end.scope))
			{
				prev=end;
				end=end.shadowed;
			}
			...
			{
				prev.shadowed=new Entry(sym,prev.shadowed,sibling,scope);
				return prev.shadowed;
			}
        }
