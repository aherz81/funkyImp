...
import com.sun.tools.javac.util.RegressionDiagnosticFormatter.BasicConfiguration;

import static com.sun.tools.javac.api.DiagnosticFormatter.PositionKind.*;
import static com.sun.tools.javac.util.RegressionDiagnosticFormatter.BasicConfiguration.*;
 ...
 * </ul>
 */
public class RegressionDiagnosticFormatter extends AbstractDiagnosticFormatter {
    ...
    public RegressionDiagnosticFormatter(Options options, JavacMessages msgs) {
        super(msgs, new BasicConfiguration(options));
        getConfiguration().setCaretEnabled(false);
    }
     ...
     * @param msgs JavacMessages object used for i18n
     */
    public RegressionDiagnosticFormatter(JavacMessages msgs) {
    ...
    public String formatDiagnostic(JCDiagnostic d, Locale l) {
        ...
        //String info=d.getType().toString();
        ...
        try {
            ...
            if (lines.length > 1 && getConfiguration().getVisible().contains(DiagnosticPart.DETAILS)) {
                ...
                for (int i = 1;i < lines.length; i++) {
                    buf.append(indent(lines[i], currentIndentation));
                }
            }
    ...
    protected String addSourceLineIfNeeded(JCDiagnostic d, String msg) {
        if (!displaySource(d))
            ...
            String sourceLine = " :\t" + formatSourceLine(d, indentSource);
    ...
    static public class BasicConfiguration extends SimpleConfiguration {
        ...
        private void initFormat() {
            availableFormats = new LinkedHashMap<BasicFormatKind, String>();
			//ALEX: (line) or :line: ???
            setFormat(BasicFormatKind.DEFAULT_POS_FORMAT, "%f(%l): >%q<%n> : %_%t : %m");
        ...
        private void initIndentation() {
            indentationLevels = new LinkedHashMap<DiagnosticPart, Integer>();
