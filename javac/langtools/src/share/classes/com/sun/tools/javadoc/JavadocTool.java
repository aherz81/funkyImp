...
public class JavadocTool extends com.sun.tools.javac.main.JavaCompiler {
    ...
    public RootDocImpl getRootDocImpl(String doclocale,
        ...
        try {
            ...
            for (List<String> it = javaNames; it.nonEmpty(); it = it.tail) {
                String name = it.head;
                if (!docClasses && (name.endsWith(".java")||name.endsWith(".funky")||name.endsWith(".f")) && new File(name).exists()) {
                ...
                } else if (name.endsWith(".java")||name.endsWith(".funky")||name.endsWith(".f")) {
    ...
    private void parsePackageClasses(String name,
        ...
        if (files == null) {
            ...
            for (JavaFileObject fo: docenv.fileManager.list(
                    location, name, EnumSet.of(JavaFileObject.Kind.SOURCE,JavaFileObject.Kind.SOURCE2,JavaFileObject.Kind.SOURCE3), false)) {
    ...
    private Map<String,List<JavaFileObject>> searchSubPackages(
            ...
            throws IOException {
        Map<String,List<JavaFileObject>> packageFiles =
                new LinkedHashMap<String,List<JavaFileObject>>();

        Map<String,Boolean> includedPackages = new LinkedHashMap<String,Boolean>();
        ...
        if (docenv.fileManager.hasLocation(StandardLocation.SOURCE_PATH)) {
            searchSubPackages(subPackages,
                    ...
                    EnumSet.of(JavaFileObject.Kind.SOURCE,JavaFileObject.Kind.SOURCE2,JavaFileObject.Kind.SOURCE3));
            searchSubPackages(subPackages,
                    includedPackages,
                    packages, packageFiles,
            ...
            searchSubPackages(subPackages,
                    ...
                    packages, packageFiles,
                    StandardLocation.CLASS_PATH,
                    EnumSet.of(JavaFileObject.Kind.SOURCE,JavaFileObject.Kind.SOURCE2,JavaFileObject.Kind.SOURCE3, JavaFileObject.Kind.CLASS));
        }
        return packageFiles;
    }
    ...
    private static boolean isValidJavaSourceFile(String file) {
        if (!file.endsWith(".java")&&!file.endsWith(".funky")&&!file.endsWith(".f")) return false;
        String clazzName;
        if (file.endsWith(".java"))
            clazzName = file.substring(0, file.length() - ".java".length());
		else if (file.endsWith(".funky"))
            clazzName = file.substring(0, file.length() - ".funky".length());
		else if (file.endsWith(".fortran"))
            clazzName = file.substring(0, file.length() - ".f".length());
		else
			clazzName = file;
