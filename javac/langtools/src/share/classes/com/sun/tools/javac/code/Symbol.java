...
public abstract class Symbol implements Element {
	...
	public boolean must_finish=false;
	...
	public void finish() throws CompletionFailure {
        if (completer != null) {
            Completer c = completer;
            completer = null;
			must_finish=true;
    ...
    public static class TypeSymbol
        ...
        public TypeSymbol(long flags, Name name, Type type, Symbol owner,boolean domain) {
            super(TYP|DOM, flags, name, type, owner);
        }
    ...
    public static class ClassSymbol extends TypeSymbol implements TypeElement {
         ...
         */
		public Set<String> taskGroups = new LinkedHashSet<String>();
		public Set<String> taskThreads = new LinkedHashSet<String>();

		public Set<JCMethodDecl> ffContext=new LinkedHashSet<JCMethodDecl>();
		...
		public ListBuffer<JCTree.JCVariableDecl> init_constructors;

		public Set<Symbol> referenced_classes = new java.util.LinkedHashSet<Symbol>();

		public VarSymbol thisSym=null;

		public boolean containsPointer()
		{
			if(members_field!=null)
			{
				for(Symbol s:members_field.getElements())
				{
					if(s instanceof VarSymbol&&((VarSymbol)s).type.containsPointer())
						return true;
				}
			}
		...
		public void recAddRef(Type t,boolean allowarray)
		{
			if(t.getArrayType().tag!=TypeTags.CLASS)
                        {
                            if(t.tag!=TypeTags.CLASS||!allowarray)
				return;
                        }

			if(referenced_classes.contains(t.tsym))
				return;

			referenced_classes.add(t.tsym);

			for(Type p:t.allparams())
			{
				if(p.tsym instanceof ClassSymbol)
					recAddRef(p,allowarray);
			}
		}
        ...
        public Type erasure(Types types) {
            if (erasure_field == null)
                erasure_field = new ClassType(types.erasure(type.getEnclosingType()),
                                              List.<Type>nil(), this);
			((ClassType)erasure_field).tree=((ClassType)type).tree;
            return erasure_field;
        }
...
//ALEX: DOMAINS
    ...
    /** A class for class symbols
     */
    public static class DomainSymbol extends TypeSymbol implements TypeElement {

        /** a scope for all class members; variables, methods and inner classes
         *  type parameters are not part of this scope
         */
        public Scope constraints_field;
		...
		public JCDiagnostic.DiagnosticPosition declarationPos=null;

        public DomainSymbol(long flags, Name name, Type type, Symbol owner) {
            super(flags, name, type, owner,true); //mark as domain
            this.constraints_field = null;
            this.fullname = formFullName(name, owner);
            this.flatname = formFlatName(name, owner);
			this.members_field = null;
            this.sourcefile = null;
            this.classfile = null;
            this.flags_field = PUBLIC;
        }

        public DomainSymbol(long flags, Name name, Symbol owner) {
            this(
                flags,
                name,
                new DomainType(Type.noType, List.<Type>nil(), null),
                owner);
            this.flags_field = PUBLIC;
            this.type.tsym = this;
        }
        ...
        public Scope constraints() {
            if (completer != null) complete();
            return constraints_field;
        }
        ...
        public Type getSuperclass() {
            ...
            if (type instanceof DomainType) {
                DomainType t = (DomainType)type;
                if (t.parentDomain == null) // FIXME: shouldn't be null
                    t.parentDomain = Type.noType;
                // An interface has no superclass; its supertype is Object.
                return t.isInterface()
                    ? Type.noType
                    : t.parentDomain;
        ...
        public boolean isSubDomain(Symbol base, Types types) {
            if (this == base) {
                return true;
            }
            else {
                for (Type t = type; t.tag == DOMAIN; t = types.supertype(t))
                    if (t.tsym == base) return true;
            }
            return false;
        }
        ...
        public void complete() throws CompletionFailure {
            try {
                ...
                // quiet error recovery
                flags_field |= (PUBLIC|STATIC);
                this.type = new DomainErrorType(this, Type.noType);
                throw ex;
            }
        }

        public Type getSuperDomain() {
            complete();
            if (type instanceof ClassType) {
                ClassType t = (ClassType)type;
                if (t.supertype_field == null) // FIXME: shouldn't be null
        ...
        public <R, P> R accept(Symbol.Visitor<R, P> v, P p) {
            return v.visitDomainSymbol(this, p);
        }
    }
...
//END DOMAINS
    ...
    public static class VarSymbol extends Symbol implements VariableElement {
        ...
        public LinkedHashMap<VarSymbol,Set<VarSymbol>> aliasMap = new LinkedHashMap<VarSymbol,Set<VarSymbol>>();
        public LinkedHashMap<VarSymbol,Set<VarSymbol>> aliasMapLinear = new LinkedHashMap<VarSymbol,Set<VarSymbol>>();
        public boolean heapAllocated = false;
//        public VarSymbol reuse = null;
//        public boolean persistent = true;
		...
		public VarSymbol getSymbol()
		{
			if(isLinear()&&isLocal())
			{
				if(aliasMapLinear.values().isEmpty()||aliasMapLinear.values().iterator().next().size()!=1)
					return this;
				else
				{
					VarSymbol s= aliasMapLinear.values().iterator().next().iterator().next();
					return s;
				}
			}
			return this;
		...
		public boolean tasklocal()
        {
            return (flags_field&TASKLOCAL)!=0;
        }

        public boolean isLinear()
        {
            return (flags_field&LINEAR)!=0;
        }
        ...
        public static Set<VarSymbol> getVarInstance(VarSymbol vs,boolean check_linear,Set<VarSymbol> res)
        {
            //Set<VarSymbol> res=new LinkedHashSet<VarSymbol>();

            if(!check_linear||(vs.flags_field&LINEAR)!=0)
            {
                Set<VarSymbol> ks=vs.aliasMap.keySet();

                for(Iterator<VarSymbol> i=ks.iterator();i.hasNext();)
                {
                    Set<VarSymbol> vss=vs.aliasMap.get(i.next());
                    //res.addAll(vss);
                    for(Iterator<VarSymbol> j=vss.iterator();j.hasNext();)
                    {
                        VarSymbol n=j.next();
                        if(n!=vs&&!res.contains(n) &&(!check_linear||(n.flags_field&LINEAR)!=0))
						{
							res.add(n);
                            res.addAll(getVarInstance(n,check_linear,res));
						}
                    }
                }
            }
        ...
        public static Set<VarSymbol> getVarInstanceLinear(VarSymbol vs,Set<VarSymbol> res)
        {
            //Set<VarSymbol> res=new LinkedHashSet<VarSymbol>();

            if((vs.flags_field&LINEAR)!=0)
            {
                Set<VarSymbol> ks=vs.aliasMapLinear.keySet();

                for(Iterator<VarSymbol> i=ks.iterator();i.hasNext();)
                {
                    Set<VarSymbol> vss=vs.aliasMapLinear.get(i.next());
                    //res.addAll(vss);
                    for(Iterator<VarSymbol> j=vss.iterator();j.hasNext();)
                    {
                        VarSymbol n=j.next();
                        if(n!=vs&&!res.contains(n) &&((n.flags_field&LINEAR)!=0))
						{
							res.add(n);
                            res.addAll(getVarInstanceLinear(n,res));
						}
                    }
                }
            }

            if(res.isEmpty())//&&(vs.flags_field&LINEAR)!=0)
                res.add(vs);
...
//          System.out.println("clone " + v + " in " + newOwner);//DEBUG
        ...
        /** Clone this symbol with new owner.
         */
        public VarSymbol clone(Name newName) {
            VarSymbol v = new VarSymbol(flags_field, newName, type, owner);
            v.pos = pos;
            v.adr = adr;
            v.data = data;
//          System.out.println("clone " + v + " in " + newOwner);//DEBUG
        ...
        public VarSymbol cloneType(Name newName,Symbol newown,int newpos) {
            VarSymbol v = new VarSymbol(flags_field, newName, type, newown);
            v.pos = pos;
            v.adr = -1;
            v.data = null;
...
//          System.out.println("clone " + v + " in " + newOwner);//DEBUG
     ...
     */
    public static class MethodSymbol extends Symbol implements ExecutableElement,Cloneable {

        /** The code of the method. */
        public Code code = null;
        public LlvmCode llvmCode = null;
		...
		public Map<VarSymbol,Set<List<JCTree.JCExpression>>> update_vars = new LinkedHashMap<VarSymbol,Set<List<JCTree.JCExpression>>>();
		public String groups = "";
		public String threads = "";
		public int LocalTGWidth = 0;
		public int NaiveTGWidth = 0;
        ...
        public boolean ReturnValueLinear=true;
        public boolean mayBeRecursive=false;

        public Set<VarSymbol> retValAlias=new LinkedHashSet<VarSymbol>();
        public Set<VarSymbol> retValAliasLinear=new LinkedHashSet<VarSymbol>();

        public Set<JCTree> implementations=new LinkedHashSet<JCTree>();

		public Map<String,MethodSymbol> cache = new LinkedHashMap<String, MethodSymbol>();

        public boolean isDomainProjection = false;
        ...
        public MethodSymbol(long flags, Name name, Type type, Symbol owner,JCTree impl) {
            super(MTH, flags, name, type, owner);
        ...
        public MethodSymbol clone(Symbol newOwner) {
            ...
            m.implementations.addAll(implementations);
            m.code = code;
            return m;
        }
        ...
        public List<VarSymbol> params() {
            ...
            if (params == null) {
                ...
                for (Type t : type.getParameterTypes()) {
					if(t.isPointer())
						buf.append(new VarSymbol(PARAMETER|FOUT, names.head, t, this));
					else
...
//          this.printStackTrace();//DEBUG
    ...
    public interface Visitor<R,P> {
        R visitDomainSymbol(DomainSymbol s, P arg);
