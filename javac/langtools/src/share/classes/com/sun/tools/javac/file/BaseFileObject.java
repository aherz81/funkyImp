...
public abstract class BaseFileObject implements JavaFileObject {
    ...
    public JavaFileObject.Kind getKind() {
        ...
        if (n.endsWith(CLASS.extension))
            return CLASS;
        if (n.endsWith(CLASS2.extension))
            return CLASS2;
        if (n.endsWith(HEADER.extension))
            return HEADER;
        else if (n.endsWith(SOURCE.extension))
            return SOURCE;
        else if (n.endsWith(SOURCE2.extension))
            return SOURCE2;
        else if (n.endsWith(SOURCE3.extension))
            return SOURCE3;
        else if (n.endsWith(HTML.extension))
            return HTML;
        if (n.endsWith(MAKEFILE.extension))
            return MAKEFILE;
        else
            return OTHER;
    }
