...
public class TreeTranslator extends JCTree.Visitor {
    ...
    private Stack<ListBuffer<JCTree>> inject_stack = new Stack<ListBuffer<JCTree>>();

    public void inject(JCTree t)
    {
        inject_stack.peek().append(t);
    }
    ...
    public <T extends JCTree> List<T> translate(List<T> trees) {
        ...
        {
            //result_list.clear();//FIXME: stack!!
            inject_stack.push(new ListBuffer<JCTree>());

            l.head = translate(l.head);

            for(List<JCTree> al=inject_stack.pop().toList();al.nonEmpty();al = al.tail)
            {
                //add injected items in proper order
                l.tail = l.tail.prepend((T)al.head);
                l = l.tail; //skip added items as they have already been translated
            }
...
//ALEX
    public void visitDomainDef(JCDomainDecl tree) {
        tree.constraints=translate(tree.constraints);
        tree.defs=translate(tree.defs);
        tree.domargs=translate(tree.domargs);
        tree.domparams=translate(tree.domparams);
        result = tree;
    }
    ...
    public void visitSet(JCSet tree) {
        List<JCExpression> l=translate(tree.getContent());
        tree.content.clear();
        tree.content.appendList(l);
        result = tree;
    }

    public void visitSequence(JCSequence tree) {
        List<JCExpression> l=translate(tree.seq);
	...
	public void visitJoin(JCJoinDomains tree) {
        List<JCExpression> l=translate(tree.doms);
		ListBuffer<JCExpression> lb=new ListBuffer<JCExpression>();
		lb.addAll(l);
        tree.doms=lb.toList();
        result = tree;
    ...
    public void visitSelectCond(JCSelectCond tree) {
        ...
        if(tree.res!=null)
            tree.res = translate(tree.res);
        else
            tree.stmnt = translate(tree.stmnt);
        result = tree;
    }

    public void visitSelectExp(JCSelect tree) {
        tree.list = translate(tree.list);
        result = tree;
    }
    ...
    public void visitDomIter(JCDomainIter tree) {
        ...
        if(tree.body!=null)
            tree.body = translate(tree.body);
        if(tree.sbody!=null)
            tree.sbody = translate(tree.sbody);
        result = tree;
    }
    ...
    public void visitDomConstraint(JCDomConstraint tree) {
        ListBuffer<JCDomConstraintValue> cvs = new ListBuffer<JCDomConstraintValue>();
        for(JCDomConstraintValue cv : tree.right)
            cvs.add(translate(cv));
        tree.right = cvs.toList();
        result = tree;
    }

    public void visitDomParameter(JCDomParameter tree) {
        result = tree;
    }

    public void visitDomConstraintValue(JCDomConstraintValue tree) {
        if(tree.coeff!=null)
            tree.coeff = translate(tree.coeff);
        if(tree.parameter!=null)
            tree.parameter = translate(tree.parameter);
        result = tree;
    }
    ...
    {
        tree.exp1 = translate(tree.exp1);

        if(tree.exp2!=null)
            tree.exp2 = translate(tree.exp2);
        result = tree;
    }
