...
public class ClassReader extends ClassFile implements Completer {
    ...
    /** A hashtable containing the encountered top-level domains,
     *  indexed by flat names. The table does not contain local classes.
     */
    private Map<Name,DomainSymbol> domains;
    ...
    private void init(Symtab syms, boolean definitive) {
        ...
        if (definitive) {
            ...
            packages = new LinkedHashMap<Name, PackageSymbol>();
            classes = new LinkedHashMap<Name, ClassSymbol>();
            domains = new LinkedHashMap<Name, DomainSymbol>();
        }
 ...
 ***********************************************************************/
    ...
    private JCDiagnostic createBadClassFileDiagnostic(JavaFileObject file, JCDiagnostic diag) {
        String key = (file.getKind() == JavaFileObject.Kind.SOURCE||file.getKind() == JavaFileObject.Kind.SOURCE2||file.getKind() == JavaFileObject.Kind.SOURCE3
 ...
 ***********************************************************************/
    ...
    Type sigToType() {
        ...
        case '[':
            sigp++;
            return new ArrayType(sigToType(),null, syms.arrayClass);
        case '(':
            sigp++;
    ...
    void readInnerClasses(ClassSymbol c) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (outer != null) { // we have a member class
                if (name == names.empty)
                    name = names.one;
                ClassSymbol member = enterClass(0,name, outer);
                if ((flags & STATIC) == 0) {
 ...
 ***********************************************************************/
    ...
    public ClassSymbol defineClass(Name name, Symbol owner) {
		return defineClass(0,name, owner);
	}
    /** Define a new class given its name and owner.
     */
    public ClassSymbol defineClass(long flags,Name name, Symbol owner) {
    ...
    public ClassSymbol enterClass(Name name, TypeSymbol owner) {
		return enterClass(0,name,owner);
	}
    /** Create a new toplevel or member class symbol with given name
     *  and owner and enter in `classes' unless already there.
     */
    public ClassSymbol enterClass(long flags,Name name, TypeSymbol owner) {
        Name flatname = TypeSymbol.formFlatName(name, owner);
        ClassSymbol c = classes.get(flatname);
        if (c == null) {
            c = defineClass(flags,name, owner);
...
//-----------------------------------------
    ...
    /** Define a new class given its name and owner.
     */
    public DomainSymbol defineDomain(Name name, Symbol owner) {
        DomainSymbol c = new DomainSymbol(0, name, owner);
        if (owner.kind == PCK)
            assert domains.get(c.flatname) == null : c;
        c.completer = this;
        return c;
    }
     ...
     *  and owner and enter in `classes' unless already there.
     */
    public DomainSymbol enterDomain(Name name, TypeSymbol owner) {
        Name flatname = TypeSymbol.formFlatName(name, owner);
        DomainSymbol c = domains.get(flatname);
        if (c == null) {
            c = defineDomain(name, owner);
            domains.put(flatname, c);
        } else if ((c.name != name || c.owner != owner) && owner.kind == TYP && c.owner.kind == PCK) {
            // reassign fields of classes that might have been loaded with
            // their flat names.
            c.owner.members().remove(c);
            c.name = name;
            c.owner = owner;
            c.fullname = DomainSymbol.formFullName(name, owner);
        }
        return c;
    }
     ...
     */
    public DomainSymbol enterDomain(Name flatName, JavaFileObject classFile) {
        DomainSymbol cs = null;
        cs = domains.get(flatName);
        if (cs != null) {
        ...
        cs = defineDomain(Convert.shortName(flatName), owner);
        cs.classfile = classFile;
        domains.put(flatName, cs);
        return cs;
    }
     ...
     */
    public DomainSymbol enterDomain(Name flatname) {
        DomainSymbol c = domains.get(flatname);
        if (c == null)
        {
            c = enterDomain(flatname, (JavaFileObject)null);
            ClassSymbol cs = classes.get(flatname);
            if(cs != null)
            {
                c.sourcefile = cs.sourcefile;
                c.classfile = cs.classfile;
    ...
    public ClassSymbol enterClass(Name flatName, JavaFileObject classFile) {
        ...
        PackageSymbol owner = packageName.isEmpty()
                                ? syms.unnamedPackage
                                : enterPackage(packageName);
        cs = defineClass(0,Convert.shortName(flatName), owner);
        cs.classfile = classFile;
    ...
    public void complete(Symbol sym) throws CompletionFailure {
        if (sym.kind == TYP) {
			...
			//ALEX: do not try to load object!
			//if(!c.name.equals(names.Object)&&!c.name.equals(names.String)&&!c.name.equals(names.Integer)&&!c.name.equals(names.Float)&&!c.name.equals(names.Double)&&!c.name.equals(names.Long))
			//if(!sym.owner==syms.)
			if((sym.type.type_flags_field&Flags.NATIVE)==0&&!sym.owner.name.equals(names.funky))
				fillIn(c);
			//else
			//	System.out.println("[Suppresed loading] "+c.name);
        }
        else if (sym.kind == (DOM|TYP)) {
            DomainSymbol c = (DomainSymbol)sym;
            //c.members_field = new Scope.ErrorScope(c); // make sure it's always defined
            boolean suppressFlush = this.suppressFlush;
            this.suppressFlush = true;
            try {
			...
			if((sym.type.type_flags_field&Flags.NATIVE)==0)
	            fillIn(c);
        }
     ...
     * possible with the Tree API or javax.lang.model.*.
     */
    private void completeEnclosing(DomainSymbol c) {
        if (c.owner.kind == PCK) {
            Symbol owner = c.owner;
            for (Name name : Convert.enclosingCandidates(Convert.shortName(c.name))) {
                Symbol encl = owner.members().lookup(name).sym;
    ...
    private void fillIn(DomainSymbol c) {
        if (completionFailureName == c.fullname) {
            throw new CompletionFailure(c, "user-selected completion failure by class name");
        }
        currentOwner = c;
     ...
     *  The class is entered into `classes' only if load was successful.
     */
    public DomainSymbol loadDomain(Name flatname) throws CompletionFailure {
        boolean absent = domains.get(flatname) == null;
        DomainSymbol c = enterDomain(flatname);
        if (c.completer != null) {
            try {
                c.complete();
            } catch (CompletionFailure ex) {
                if (absent) domains.remove(flatname);
                throw ex;
            }
        }
        return c;
 ...
 ***********************************************************************/
    ...
    protected JavaFileObject preferredFileObject(JavaFileObject a,
        ...
        if (preferSource)
            return (a.getKind() == JavaFileObject.Kind.SOURCE||a.getKind() == JavaFileObject.Kind.SOURCE2||a.getKind() == JavaFileObject.Kind.SOURCE3) ? a : b;
        else {
    ...
    protected EnumSet<JavaFileObject.Kind> getPackageFileKinds() {
        return EnumSet.of(JavaFileObject.Kind.CLASS, JavaFileObject.Kind.SOURCE, JavaFileObject.Kind.SOURCE2, JavaFileObject.Kind.SOURCE3);
    }
...
//ALEX: fileManager.list retrieves installed files for package name, fillin loads all the files
        ...
        classKinds.remove(JavaFileObject.Kind.SOURCE2);
        classKinds.remove(JavaFileObject.Kind.SOURCE3);
    ...
    public interface SourceCompleter {
        ...
        void complete(DomainSymbol sym)
            throws CompletionFailure;
    }
