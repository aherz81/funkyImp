 ...
 ****************************************************************************/
    ...
    public R visitArgExpression(ArgExpressionTree node, P p) {
        R r = scan(node.getExpression1(), p);
        r = scanAndReduce(node.getExpression2(), p, r);
        return r;
    }

    //ALEX
    public R visitDomain(DomainTree node, P p) {
        R r = scan(node.getDomParameters(), p);
        r = scanAndReduce(node.getConstraints(), p, r);
        r = scanAndReduce(node.getDefs(), p, r);
        r = scanAndReduce(node.getDomArgs(), p, r);
        return r;
    }

    public R visitDomIter(DomIterTree node, P p) {
        R r = scan(node.getDomArgs(), p);
        r = scanAndReduce(node.getExpression(), p, r);
        r = scanAndReduce(node.getBody(), p, r);
        return r;
    }

    public R visitDomConstraint(DomConstraintTree node, P p) {
        R r = scan(node.getRightSide(), p);
        //operator, etc??
        return r;
    }

    public R visitDomConstraintValue(DomConstraintValueTree node, P p) {
        R r = scan(node.getCoeff(), p);
        r = scanAndReduce(node.getParameter(), p, r);
        return r;
    }

    public R visitDomParameter(DomParameterTree node, P p) {
        R r = null;//scan(node., p);
        return r;
    }

    public R visitCTProperty(CTPropertyTree node, P p) {
        R r = null;//scan(node., p);
        return r;
    }

    public R visitDomInstance(DomInstanceTree node, P p) {
        //node.getName()
        R r = null;//scan(node., p);
        return r;
    }

	public R visitDomParent(DomParentTree node, P p) {
        //node.getName()
        R r = null;//scan(node., p);
        return r;
    }

	public R visitDomUsage(DomUsageTree node, P p) {
        //node.getName()
        R r = null;//scan(node., p);
        return r;
    }
    ...
    public R visitCF(EmptyStatementTree node, P p) {
        return null;
    }

    public R visitSet(SetTree node, P p) {
	...
	public R visitSequence(SequenceTree node, P p) {
        return scan(node.getList(), p);
    }

	public R visitJoin(JoinTree node, P p) {
        return scan(node.getOperands(), p);
    }
    ...
    public R visitIfExp(IfExpTree node, P p) {
        R r = scan(node.getCondition(), p);
        r = scanAndReduce(node.getThenExp(), p, r);
        r = scanAndReduce(node.getElseExp(), p, r);
        return r;
    }
    ...
    public R visitPragma(PragmaTree node, P p) {
        R r = scan(node.getCondition(), p);
        r = scanAndReduce(node.getDetail(), p, r);
        return r;
    }
    ...
    public R visitWhere(WhereTree node, P p) {
        R r = scan(node.getExpression(), p);
        r = scanAndReduce(node.getStatement(), p, r);
        r = scanAndReduce(node.getSExpression(), p, r);
        return r;
    }

    public R visitFor(ForTree node, P p) {
        R r = scan(node.getExpression(), p);
        r = scanAndReduce(node.getContent(), p, r);
        return r;
    }

    public R visitSelectExp(SelectTree node, P p) {
        R r = scan(node.getCondList(), p);
        return r;
    }

    public R visitCaseExp(CaseExpTree node, P p) {
        R r = scan(node.getCondList(), p);
        r = scanAndReduce(node.getExp(), p, r);
        return r;
    }

    public R visitSelectCond(SelectCondTree node, P p) {
        R r = scan(node.getCond(), p);
        r = scanAndReduce(node.getRes(), p, r);
        return r;
    }
    ...
    public R visitUnary(UnaryTree node, P p) {
        R r = scan(node.getExpression(), p);
        r = scanAndReduce(node.getApply(), p, r);
        return r;
    }
	...
	public R visitSizeOf(SizeOfTree node, P p) {
        return scan(node.getExpression(), p);
    }
