...
//ocl_device ocl_setup::getBestDevice() {
//    
//    std::vector<std::tuple<std::string, int>> deviceSpeeds;
//    for (int i=0; i < pSize; i++) {
//        for (int j=0; j < dSize[i]; j++) {
//            //cl_
//        }
...
void ocl_kernel::run(){
    ...
    //std::cout << c_lDims[0] << " " << c_lDims[1] << " " << c_lDims[2] << " " << std::endl;
    //std::cout << c_gDims[0] << " " << c_gDims[1] << " " << c_gDims[2] << " " << std::endl;
...
int ocl_kernel::getOptimalWgSize(size_t numberOfWorkItems) {
    size_t maxWgSize = device->getMaxWorkgroupSize();
    for (size_t x = maxWgSize; x > 0; x--) {
        if (numberOfWorkItems % x == 0) {
            return x;
        }
    }
    return 1;
...
cl_uint ocl_device::getClockFrequency()
{
    cl_uint clockFrequency;
    clGetDeviceInfo(this->getDeviceID(), CL_DEVICE_MAX_CLOCK_FREQUENCY, sizeof(cl_uint), &clockFrequency, NULL);
    return clockFrequency;
}

size_t ocl_device::getMaxWorkgroupSize()
{
    size_t maxWorkgroupSize;
    clGetDeviceInfo(this->getDeviceID(), CL_DEVICE_MAX_WORK_GROUP_SIZE, sizeof(size_t), &maxWorkgroupSize, NULL);
    return maxWorkgroupSize;    
}

cl_uint ocl_device::getMaxComputeUnits()
{
    cl_uint maxComputeUnits;
    clGetDeviceInfo(this->getDeviceID(), CL_DEVICE_MAX_COMPUTE_UNITS, sizeof(cl_uint), &maxComputeUnits, NULL);
    return maxComputeUnits;
}

cl_ulong ocl_device::getMaxMemoryAllocationSize()
{
    cl_ulong maxAllocSize;
    clGetDeviceInfo(this->getDeviceID(), CL_DEVICE_MAX_MEM_ALLOC_SIZE, sizeof(cl_ulong), &maxAllocSize, NULL);
    return maxAllocSize;
}
//-----------------------------//
//         OCL_CONTEXT         //
